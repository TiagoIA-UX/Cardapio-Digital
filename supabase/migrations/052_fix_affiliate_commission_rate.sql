...
BEGIN
  ...
  SET
    ...
    comissao            = COALESCE(ar.comissao, ROUND(
      p_valor_assinatura * COALESCE(
        (SELECT a.commission_rate FROM public.affiliates a WHERE a.id = ar.affiliate_id),
        0.30
      ), 2
    )),
  ...
  WHERE ar.tenant_id = p_tenant_id
    ...
    AND (ar.referencia_mes IS NULL OR ar.referencia_mes = v_ref_mes);
