...
CREATE TABLE restaurants (
  ...
  banner_url TEXT,
  slogan VARCHAR(255),
  template_slug VARCHAR(30) DEFAULT 'restaurante',
  google_maps_url TEXT,
  endereco_texto TEXT,
  customizacao JSONB DEFAULT '{}'::jsonb,
...
-- Usando tabela de sequência + upsert atômico
-- =============================================
...
CREATE OR REPLACE FUNCTION get_next_order_number(p_restaurant_id UUID) RETURNS INTEGER AS $$
DECLARE next_num INTEGER;
BEGIN
INSERT INTO order_number_sequences AS seq (restaurant_id, last_value, updated_at)
...
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_restaurants_updated BEFORE
UPDATE ON restaurants FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER trigger_products_updated BEFORE
UPDATE ON products FOR EACH ROW EXECUTE FUNCTION update_updated_at();
CREATE TRIGGER trigger_orders_updated BEFORE
...
CREATE POLICY "Restaurantes públicos são visíveis" ON restaurants FOR
SELECT USING (ativo = true);
CREATE POLICY "Dono pode ver seu restaurante" ON restaurants FOR
SELECT USING (auth.uid() = user_id);
CREATE POLICY "Dono pode editar seu restaurante" ON restaurants FOR
UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Usuário autenticado pode criar restaurante" ON restaurants FOR
INSERT WITH CHECK (auth.uid() = user_id);
-- PRODUCTS: Públicos se restaurante ativo, dono pode CRUD
CREATE POLICY "Produtos de restaurantes ativos são visíveis" ON products FOR
SELECT USING (
    ativo = true
    AND EXISTS (
      SELECT 1
...
CREATE POLICY "Dono pode ver todos seus produtos" ON products FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM restaurants
...
CREATE POLICY "Dono pode criar produtos" ON products FOR
INSERT WITH CHECK (
    EXISTS (
      SELECT 1
      FROM restaurants
...
CREATE POLICY "Dono pode editar produtos" ON products FOR
UPDATE USING (
    EXISTS (
      SELECT 1
      FROM restaurants
...
CREATE POLICY "Dono pode deletar produtos" ON products FOR DELETE USING (
  EXISTS (
    SELECT 1
    FROM restaurants
    WHERE restaurants.id = products.restaurant_id
...
CREATE POLICY "Qualquer um pode criar pedido" ON orders FOR
INSERT WITH CHECK (true);
CREATE POLICY "Dono pode ver pedidos do restaurante" ON orders FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM restaurants
...
CREATE POLICY "Dono pode atualizar pedidos" ON orders FOR
UPDATE USING (
    EXISTS (
      SELECT 1
      FROM restaurants
...
CREATE POLICY "Qualquer um pode criar items de pedido" ON order_items FOR
INSERT WITH CHECK (true);
CREATE POLICY "Dono pode ver items dos pedidos" ON order_items FOR
SELECT USING (
    EXISTS (
      SELECT 1
      FROM orders
...
CREATE TABLE IF NOT EXISTS template_order_items (
  ...
  template_id UUID REFERENCES templates(id) ON DELETE
  SET NULL,
...
CREATE TABLE IF NOT EXISTS user_purchases (
  ...
  template_id UUID REFERENCES templates(id) ON DELETE
  SET NULL,
    order_id UUID REFERENCES template_orders(id) ON DELETE
  SET NULL,
...
CREATE POLICY "Templates são públicos" ON templates FOR
SELECT USING (true);
CREATE POLICY "Usuários podem gerenciar próprio carrinho" ON cart_items FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Usuários podem ver próprios pedidos" ON template_orders FOR
SELECT USING (auth.uid() = user_id);
CREATE POLICY "Usuários podem ver próprias compras" ON user_purchases FOR
SELECT USING (auth.uid() = user_id);
CREATE POLICY "Reviews são públicas" ON reviews FOR
SELECT USING (true);
CREATE POLICY "Usuários podem criar reviews" ON reviews FOR
INSERT WITH CHECK (auth.uid() = user_id);
-- =============================================
-- FUNÇÕES: E-COMMERCE
-- =============================================
-- Gerar license key
CREATE OR REPLACE FUNCTION generate_license_key() RETURNS TEXT AS $$ BEGIN RETURN upper(
    substr(md5(random()::text), 1, 4) || '-' || substr(md5(random()::text), 1, 4) || '-' || substr(md5(random()::text), 1, 4) || '-' || substr(md5(random()::text), 1, 4)
  );
END;
$$ LANGUAGE plpgsql;
-- Trigger para gerar license key
CREATE OR REPLACE FUNCTION set_license_key() RETURNS TRIGGER AS $$ BEGIN IF NEW.license_key IS NULL THEN NEW.license_key := generate_license_key();
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trigger_set_license_key ON user_purchases;
CREATE TRIGGER trigger_set_license_key BEFORE
INSERT ON user_purchases FOR EACH ROW EXECUTE FUNCTION set_license_key();
-- Incrementar uso do cupom
CREATE OR REPLACE FUNCTION increment_coupon_usage(p_coupon_id UUID) RETURNS VOID AS $$ BEGIN
UPDATE coupons
...
CREATE OR REPLACE FUNCTION user_has_template_access(p_user_id UUID, p_template_id UUID) RETURNS BOOLEAN AS $$ BEGIN RETURN EXISTS (
    SELECT 1
    FROM user_purchases
...
BEGIN new_number := 'ORD-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || LPAD(FLOOR(RANDOM() * 10000)::TEXT, 4, '0');
RETURN new_number;
END;
$$ LANGUAGE plpgsql;
-- =============================================
