...
async function post(
  ...
  try {
    ...
    results.push({ label, status: res.status, body: text, passed, reason: passed ? 'ok' : `esperado ${expectStatus}` })
    const icon = passed ? '✅' : '❌'
...
async function get(label: string): Promise<void> {
  ...
  try {
    ...
    results.push({ label, status: res.status, body: text, passed, reason: passed ? 'ok' : `HTTP ${res.status}` })
...
async function main() {
  ...
  console.log(`  Secret : ${SECRET ? '✅ configurado (validação HMAC ativa)' : '⚠️  sem MP_WEBHOOK_SECRET (validação HMAC ignorada)'}`)
  console.log('═'.repeat(60))
  console.log()
  ...
  const dupBody = { id: dupId, type: 'subscription_preapproval', data: { id: 'sub-dup-001' }, live_mode: false }
  console.log()
  ...
  else if (ignoredFails.length > 0) console.log('  ℹ️  T6 falhou como esperado (ID de pagamento inválido no MP)')
}
