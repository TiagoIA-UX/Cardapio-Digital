...
const KNOWN_PATTERNS: KnownPattern[] = [
  ...
  {
    pattern:
      'ESLint: diagnóstico de cache desatualizado — erro reportado ainda aparece após correção',
    error_type: 'custom',
    root_cause:
...
async function run() {
  ...
  if (isDryRun) {
    for (const p of KNOWN_PATTERNS) {
      console.log(
        `  [DRY] ${p.error_type.padEnd(12)} | auto:${String(p.auto_fixable).padEnd(5)} | conf:${p.confidence}% | ${p.pattern.slice(0, 70)}`
      )
    }
